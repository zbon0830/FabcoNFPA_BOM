...
male_rod_codes = {
    ...
    "MJ","MK","MQ","MR","PM","SL","SM","VB","VC","VD","VE","VF","VG",
...
def rear_cover_calc(bore, mounting, ports, cushions, options, rod_style):
    ...
    cover_code = {
        ...
        'X2': '100', 'X3': '100', 'S2': '130', 'S4': '110', 'E3': '100', 'E4': '190',
...
def rod_wiper_calc(bore, rod_style, options):
    ...
    if bore in ('15', '20', '25'):
        if options in metallic_scraper_codes:
            rod_wiper = 'SG-3435-5/8-BN' if rod_style in ('1', '2', '3') else 'SG-3436-1-BN'
            if options in viton_codes:
...
def retaining_ring_calc(bore, options, rod_style):
    ...
    if bore in ('15', '20', '25'):
        ...
        if rod_style in ('1', '2', '3'):
            ...
            retaining_ring = 'N5000-237PA' if options not in stainless_fastener_codes else 'N5000-237SS'
    elif bore in ('60', '80'):
        if rod_style in ('1', '2', '3'):
            retaining_ring = 'N5000-237PA' if options not in stainless_fastener_codes else 'N5000-237SS'
    if options in double_rod_codes:
...
def pivot_calc(bore, mounting, options):
    ...
    if mounting in ('P1', 'P2'):
        if bore in ('15', '20', '25'):
            pivot_pin = 'HP-11-155' if options not in stainless_fastener_codes else 'N15-500SS'
            pivot_ring = 'N5100-50' if options not in stainless_fastener_codes else 'N5100-50SS'
            pivot_ring_qty = 2
        elif bore in ('32', '40', '50'):
            pivot_pin = 'HP-30-155' if options not in stainless_fastener_codes else 'N32-750SS'
            pivot_ring = 'N5100-75' if options not in stainless_fastener_codes else 'N5100-75SS'
            pivot_ring_qty = 2
        elif bore in ('60', '80'):
            pivot_pin = 'HP-60-155' if options not in stainless_fastener_codes else 'N60-100SS'
...
def piston_bolt_calc(bore, options, rod_style, cushions):
    ...
    if options not in double_rod_codes:
        if bore in ('15','20','25','30'):
            ...
            if rod_style in ('1', '2', '3'):
                piston_bolt = 'M14X2.0X55SHCS' if cushions == 'A' else 'M14X2.0X65SHCS'
            else:
...
def generate_bom(parsed_data):
    ...
    if mounting == 'T8': #recalls the tie rod calculation with new parameters to fit mid-trunnion (WIP)
        ...
        tie_rod_2, tie_rod_2_qty = tie_rod_calc(bore, options, stroke, 'T8_2', fractional_stroke, rod_style,xi_num)
